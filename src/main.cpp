...
void opcontrol() {
  ...
  while (true) {
    ...
    // // ---- Stall detection ----
    // if (cmd != 0 && fabs(levermotor.get_actual_velocity()) < 5) {
    //   lever_stall_time += 20;  // opcontrol loop ~20ms
    // } else {
    //   lever_stall_time = 0;
    // }

    // // Stop motor ONLY if stalled long enough
    // if (lever_stall_time > 150) {  // ~0.15 sec
    //   levermotor.move(0);
    // }
    ...
    // Stall detection
    if (rewinding && cmd != 0 && fabs(levermotor.get_actual_velocity()) < 5) {
      lever_stall_time += ez::util::DELAY_TIME;
    } else {
      lever_stall_time = 0;
    }

    // Home reached
    if (rewinding && lever_stall_time > 150) {
      levermotor.move(0);
      levermotor.set_brake_mode(MOTOR_BRAKE_COAST);
    ...
    scraper.button_toggle(master.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_B));
    lift.button_toggle(master.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_RIGHT));
    hood.button_toggle(master.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_X));
